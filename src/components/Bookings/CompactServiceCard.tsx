						...
						<div>
							<h3 className='font-bold text-gray-900 text-base'>
								{request?.customerId?.firstName +
									' ' +
									request?.customerId?.lastName || 'cusotmer name'}
							</h3>
				...
				<div className='grid grid-cols-2 gap-3 mb-3'>
					<div>
						...
						<p className='font-semibold text-gray-900 text-sm'>
							{request?.customerId?.vehicleInfo?.model || 'vehicle model'}
						</p>
						<p className='font-semibold text-gray-900 text-sm'>{'2025'}</p>
						<p className='font-mono text-black rounded mt-1 text-sm font-medium'>
					...
					<div className='flex flex-wrap gap-1'>
						{request?.service
							...
							.map((service: any, index: number) => (
								<span
									key={index}
									className='bg-[#9b111e] text-white px-2 py-1 rounded text-sm font-medium'
								>
