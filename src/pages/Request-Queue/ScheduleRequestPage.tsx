...
//       const customerName = `${req.customerId?.firstName || ''} ${req.customerId?.lastName || ''}`.toLowerCase();
...
//                     />
//                   </svg>
//                   <span>{new Date(req.schedule_date).toLocaleDateString()}</span>
//                 </div>
...
//                     Time: {req.preferedTime.startTime} - {req.preferedTime.endTime}
//                   </div>
//                 )}
//               </div>
//             </div>
  ...
  const assignPartner = async () => {
    ...
    try {
      ...
      const updatedRequest = {
        ...
        status: "Scheduled",
        assignedPartner: selectedPartnerId,
        partnerName: `${selectedPartner?.firstName} ${selectedPartner?.lastName}` || "Unknown Partner"
      };

      await UpdateScheduleReq({ partnerId: selectedPartnerId }, selectedRequest._id);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRequests.map((req, index) => (
            ...
            >
              <div className="flex items-center gap-6">
                <div className="w-10 h-10 bg-[#9b111e] rounded-full flex items-center justify-center text-white font-bold text-base">
                  {req.customerId?.firstName?.charAt(0)}{req.customerId?.lastName?.charAt(0)}
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex justify-end">
          <div className="w-full sm:w-1/2 h-full bg-[#FAF3EB] border-l-4 border-[#9b111e] shadow-2xl p-8 relative overflow-y-auto rounded-l-3xl transition-all duration-300">
            ...
            <div className="flex gap-4">
              <div className="flex-shrink-0">
                <div className="w-16 h-16 rounded-full bg-[#9b111e]/10 text-[#9b111e] text-3xl flex items-center justify-center font-bold shadow-md">
                  {selectedRequest.customerId?.firstName?.charAt(0)}{selectedRequest.customerId?.lastName?.charAt(0)}
                </div>
              </div>
                ...
                <div>
                  ...
                  {selectedPartner && (
                    <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                      <h3 className="font-semibold text-[#9b111e] mb-2" style={{ ...FONTS.cardSubHeader }}>
                        Selected Partner Details:
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                        <p>
                          <span className="font-medium">Name:</span> {selectedPartner.firstName} {selectedPartner.lastName}
                        </p>
                        <p>
                          <span className="font-medium">Phone:</span> {selectedPartner.contact_info.phoneNumber}
                        </p>
                        <p>
                          <span className="font-medium">City:</span> {selectedPartner.contact_info.city}
                        </p>
                        <p>
                          <span className="font-medium">State:</span> {selectedPartner.contact_info.state}
                        </p>
                        <p className="md:col-span-2">
                          <span className="font-medium">Address:</span> {selectedPartner.contact_info.address1}
                        </p>
                      </div>
                    </div>
                  )}
