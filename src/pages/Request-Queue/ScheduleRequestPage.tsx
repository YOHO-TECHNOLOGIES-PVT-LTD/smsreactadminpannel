...
export default function ScheduleRequestPage() {
	...
	const assignPartner = async () => {
		...
		alert(
			`Successfully assigned ${selectedPartner?.firstName} to ${selectedRequest.customerId.firstName}'s request!`
		);
		closeModal();
	};
				...
				<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
					{requests?.map((req, index) => (
						...
						>
							{/* Status Ribbon */}
							<div
								className='absolute top-0 right-0 bg-[#9b111e] !text-white text-xs font-bold px-3 py-1 rounded-bl-xl z-10'
								style={{ ...FONTS.paragraph }}
							>
								pending
							</div>
							...
							<div className='flex gap-4 p-6'>
								...
								<div className='flex-1 space-y-1'>
									...
									>
										<span className='font-medium'>🚘</span>{' '}
										{req?.customerId?.vehicleInfo?.model} •{' '}
							...
							<div className='flex-1 space-y-4 text-gray-700'>
								<div className='grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 bg-white p-6 rounded-2xl border border-gray-200 shadow-lg'>
									<p
										...
										style={{ ...FONTS.cardSubHeader }}
									>
										<span className='' style={{ ...FONTS.cardSubHeader }}>
											Customer:
										</span>{' '}
									...
									>
										...
										{selectedRequest.customerId.contact_info.address1 +
											' ' +
											selectedRequest.customerId.contact_info.address2}
									</p>
									<p
										className='!text-gray-800 text-lg'
										style={{ ...FONTS.cardSubHeader }}
