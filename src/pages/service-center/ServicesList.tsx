  ...
  return (
    ...
    <div>
        ...
        <div className="flex p-4 pb-8 sticky top-0  rounded-t-xl shadow-inner-top border-b-2 border-orange-700" style={{ backgroundColor: COLORS.bgColor }}>
                      ...
                      <h1 className="font-bold text-3xl pt-2 pl-0" style={{ color: "#9b111e" }}>Services Lists Management System</h1><form className="w-2/3 mx-auto pl-96">   
    ...
    <div className="grid grid-cols-2 gap-6 text-2xl mt-10 p-5" style={{fontFamily:FONTS.header.fontFamily}}>
     ...
     {/* <div className="w-full w-5/6 p-4 bg-white border border-gray-200 rounded-lg shadow-sm sm:p-8 dark:border-gray-700">
      <div className="flex items-center justify-between mb-4 cursor-pointer" >
        ...
        <span className="text-sm font-medium text-orange-800 hover:underline "  onClick={() => setIsOpen(!isOpen)}>
          {isOpen ? 'Hide' : 'View all'}
        </span>
      </div>
      ...
      {isOpen && (
        <div className="overflow-x-auto">
          ...
          <table className="min-w-full text-left text-sm font-light text-gray-900 dark:text-white">
            ...
            <tbody>
              {washingOptions.map((option, index) => (
                <tr key={index} className="border-b dark:border-gray-700">
                  <td className="px-0 py-0 text-2xl font-normal">{option.name}</td>
                  <td className="px-4 py-2 text-2xl font-normal">{option.price}</td>
                  <td className="px-4 py-2">
